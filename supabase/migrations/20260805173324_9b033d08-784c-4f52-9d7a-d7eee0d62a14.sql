...
BEGIN
  ...
  -- Total team size counts the lead: 1 lead + N additional members, 2..4 total.
  v_total := jsonb_array_length(v_members) + 1;
  IF v_total < 2 OR v_total > 4 THEN
  ...
  LOOP
    ...
    VALUES (
      ...
      nullif(btrim(coalesce(v_member->>'college_id_number', '')), ''),
      'member'
    );
  END LOOP;
